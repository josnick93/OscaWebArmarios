...
<div class="text-left w-100 h-100 my-2 text-white py-5 text-uppercase">
    <div class="row">
        <div class="container" style="height:85%;margin-top:5%;margin-left:3%">
            <h1 class="my-3">CONSULTA MATERIAL ${lanzadera.descripcion}</h1>
            <table class="table table-bordered table-responsive">
                ...
                <c:if test="${!empty material}">
                    <tbody>
                    <c:forEach items="${material}" var="caja">
                        <tr>
                            <td class="m-0">${caja.entablado.id_entablado}</td>
                            <td class="m-0">${caja.entablado.descripcion}</td>
                            <td class="text-left">${caja.columna}</td>
                            <td class="text-left">${caja.fila}</td>
                            <td class="text-left">${caja.tipoCaja.descripcion}</td>
                            <td class="text-left">${caja.articulo.articulo}</td>
                            <td class="text-left">${caja.cantidad}</td>
                            <td class="text-left">${caja.peso}</td>
                            <td class="text-center" style="width:5%">
                                <a id='acceder' class="btn btn-primary btn-lg outline px-3 mx-3" data-toggle="modal" style='background:#324891'
                                   href='/OscaArmarios/ConsultaEntabladoCajaVacia.html?lanzadera=${caja.lanzadera.id_lanzadera}&entablado=${caja.entablado.id_entablado}&fila=${caja.fila}&columna=${caja.columna}'>
                ...
                <c:if test="${!empty materialreserva}">
                    <tbody>
                    <c:forEach items="${materialreserva}" var="caja">
                        <tr>
                            <td class="m-0">${caja.entablado.id_entablado}</td>
                            <td class="m-0">${caja.entablado.descripcion}</td>
                            <td class="text-left">${caja.columna}</td>
                            <td class="text-left">${caja.fila}</td>
                            <td class="text-left">A MEDIDA ${caja.columna}x${caja.fila}</td>
                            <td class="text-left">${caja.articulo.articulo}</td>
                            <td class="text-left">${caja.cantidad}</td>
                            <td class="text-left">${caja.peso}</td>
                            <td class="text-center" style="width:5%">
                                <a id='acceder' class="btn btn-primary btn-lg outline px-3 mx-3" data-toggle="modal" style='background:#324891'
                                   href='/OscaArmarios/ConsultaEntabladoCajaVaciaMedida.html?lanzadera=${caja.lanzadera.id_lanzadera}&entablado=${caja.entablado.id_entablado}&fila=${caja.fila}&columna=${caja.columna}'>
                                    <span class="icon-flipped glyphicon glyphicon-search" style="vertical-align:middle;font-size: 28px;color: #FFFFFF;"/></a>
                            </td>
                        </tr>
                    </c:forEach>
