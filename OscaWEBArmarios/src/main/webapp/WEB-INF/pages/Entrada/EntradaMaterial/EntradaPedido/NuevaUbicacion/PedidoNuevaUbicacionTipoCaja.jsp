...
<div class="container" style="width: 100%;">
    <h3 class="text-left" style="margin-left: 20px;">NUEVA UBICACION - ENTRADA PEDIDO</h3>
    <div class="row">
        <div class="col-sm-2">
...
<script>
    ...
    $(document).ready(function() {
        ...
        $("#botonUbicarMedida").click(function(){
           //comprobar dimensiones
            if($('#nfilas').val() <= 0 || $('#nfilas').val() >8 || $('#nfilas').val == 7
                ...
                window.location.href = "EntradaArticuloPedidoSeleccionNuevaUbicacionFinalMedida.html?articulo=${articulo.no}&cantidad=" +parseInt(${cantidad})  +
                    "&lanzadera="+parseInt(${lanzadera.id_lanzadera})+"&entablado=PLNZ."+("00" + ${lanzadera.id_lanzadera}).slice(-2)+"."+("0000" + $('#medidaEntablado').val()).slice(-4)+"&cajafila="
                    +$('#nfilas').val()+"&cajacolumna="+$('#ncolumnas').val()+"&cerrar=-1&pedido=${pedido.no}&linea=${lineaPedido.lineNo}";
            }
        ...
        $(document).on('click', '.done', function () {
           // if((${articulo.netWeight*cantidad})>0.0){
                //aceder a la lanzadera
                if(${cerrar==1}){
                   // window.location.href = "EntradaSeleccionNuevaUbicacionFinal.html?articulo=${articulo.no}&cantidad=" +parseInt(${cantidad}) + "&caja="+  tipoUbicacion.tipoCaja +"&lanzadera="+parseInt(${lanzadera.id_lanzadera})+"&entablado="+parseInt($('#text-basic').val())+"&cerrar=1";
                    window.location.href = "EntradaArticuloPedidoSeleccionNuevaUbicacionFinal.html?articulo=${articulo.no}&cantidad=${lineaPedido.qtyOutstanding}&caja="+
                        tipoUbicacion.tipoCaja +"&entablado=PLNZ."+("00" + ${lanzadera.id_lanzadera}).slice(-2)+"."+("0000" + $('#text-basic').val()).slice(-4)+"&cerrar=-1&pedido=${pedido.no}&linea=${lineaPedido.lineNo}";

                }
                else{
                    window.location.href = "EntradaArticuloPedidoSeleccionNuevaUbicacionFinal.html?articulo=${articulo.no}&cantidad=" +parseInt(${cantidad}) + "&caja="+  tipoUbicacion.tipoCaja +"&entablado=PLNZ."+("00" + ${lanzadera.id_lanzadera}).slice(-2)+"."+("0000" + $('#text-basic').val()).slice(-4)
                        +"&cerrar=-1&pedido=${pedido.no}&linea=${lineaPedido.lineNo}";
                }
           // }
        });
    });
