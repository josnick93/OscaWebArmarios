...
<div class="container" style="width: 100%;">
    <div class="form-group form-group-sm">
        <h4>
            <span class="right" style="float:right;">PEDIDO ${pedido.no}</span>
            <span class="left"  style="float:left;">ENTABLADO ${entablado.codigo}</span>
        </h4>
    </div>
    </br>
    <div class="row">
        ...
        <div class="col-sm-10">
            ...
            <div id='entradaManual'>
                <h4  class="text-left">REUBICAR PEDIDO</h4>
            </div>
            ...
            <c:forEach items="${ubicaciones}" var="ubicacion">
                <c:choose>
                    <c:when test="${ubicacion.tipoCaja.id_caja==1 && ubicacion.fila==localizacionArticulo.fila && ubicacion.columna==localizacionArticulo.columna}">
                        <div  id='entradaArticulo' fila=${ubicacion.fila} fila2=${ubicacion.fila} columna=${ubicacion.columna} columna2=${ubicacion.columna} class="wrap cajaOcupada" style='margin-left:0.2%;top:${ubicacion.yPos}%;left:+${ubicacion.xPos}%;width:6.1%;height:11.5%;border:2px solid black;background:#dadada'><h4 class='desc'>
                    ...
                    <c:when test="${ubicacion.tipoCaja.id_caja==2 && ubicacion.fila==localizacionArticulo.fila && ubicacion.columna==localizacionArticulo.columna}">
                        <div  id='entradaArticulo' fila=${ubicacion.fila} fila2=${ubicacion.fila-1} columna=${ubicacion.columna} columna2=${ubicacion.columna} class="wrap cajaOcupada" style='margin-left:0.2%;top:${ubicacion.yPos}%;left:+${ubicacion.xPos}%;width:6.1%;height:23%;border:2px solid black;background:#dadada'><h4 class='desc'>
                                ${ubicacion.cantidad}</h4></div>
                    </c:when>
        ...
        <div class="modal-dialog">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body"> <strong>Instrucciones:</strong>
                    <p/>
                    <p>1- Espere a que se acabe de crear la ubicación</p>
    ...
    <script>
        ...
        $("#anteriorUicacion").click(function() {
            if(nPosicion>=1){
                ...
                window.location.href = "EntradaArticuloPedidoReubicarSugerirUbicacionFinal.html?articulo=${articulo.no}&caja="
                    +  ${TipoCaja.id_caja} +"&lanzadera=${lanzadera.id_lanzadera}&nsugerencia=${nsugerencia-1}&cerrar=${cerrar}&pedido=${pedido.no}&lineaPedido=${lineaPedido.lineNo}&fila=${fila}&columna=${columna}";
                }
            }else{
        ...
        $("#siguienteUbicacion").click(function() {
            ...
            if(nPosicion==1){
                ...
                if(${posiblesPosiciones!=[]}){
                    modoSubdivision=false;
                    listaUbicaciones=${posiblesPosiciones};
                    nPosicion=0;
                }else{
                    window.location.href = "EntradaArticuloPedidoReubicarSugerirUbicacionFinal.html?articulo=${articulo.no}&caja="
                        +  ${TipoCaja.id_caja} +"&lanzadera=${lanzadera.id_lanzadera}&nsugerencia=${1+nsugerencia}&cerrar=${cerrar}&pedido=${pedido.no}&lineaPedido=${lineaPedido.lineNo}&fila=${fila}&columna=${columna}";
                }
        ...
        $("#CrearUbicacion").click(function() {
            ...
            if($('#nuevaCaja').css('border-style') == 'solid'){
                $.ajax({
                    ...
                    url: '/OscaArmarios/Operacion/CrearUbicacionReubicar',
                    //lanzadera entablado x y articulo cantidad
                    data: { pedido:'${pedido.no}' ,linea:parseInt(${lineaPedido.lineNo})
                        ,columnaOrigen:parseInt(${columna})
                        ,filaOrigen:parseInt(${fila})
                        ,lanzadera:parseInt(${lanzadera.id_lanzadera})
                        ,entablado:'${entablado.codigo}'
                        ,xPos:parseInt(fila)
                        ,yPos: parseInt(columna)
                        ,articulo:'${articulo.no}'
                        ,cantidad:parseInt(${lineaPedido.qtyOutstanding})
                        ,caja:parseInt(auxCaja) , xCoord: parseFloat(xCoord)
                        ,yCoord:parseFloat(yCoord)
                        ,origen: '${lineaPedido.binCode}'
                    },
                    dataType: 'json',
                    async : true,
                    beforeSend: function()
                    ...
                    success: function(data){
                        ...
                        $( "#entradaManual" ).replaceWith(" <h4  class='text-left'>ENTRADA PEDIDO - MATERIAL AÑADIDO</h4>");

                        $("#entradaNoConfirmada").replaceWith("<h2><span class='right' style='float:right;'>" +
                            "<div class='btn btn-default btn-lg' style='background:#009900;border-radius:0;color:white'><a class='nav-link' href='http://192.168.2.8/OscaArmarios/EntradaArticuloPedido.html?pedido=${articuloPedido.no}&cerrar=${cerrar}' style='color:white'>CONTINUAR</a></div>" +
                            "</span>" +
                            "<span class='left'  style='float:left;'>" +
                            "ENTRADA CONFIRMADA DE ${lineaPedido.qtyOutstanding} UDS EN LA UBICACION "+ fila+" , "+ columna+"</span></h2>");
                            dibujarEntablado();
                        //liberar lanzadera
                    },
                    error: function(data)
        ...
        function dibujarEntablado() {
            ...
            switch(auxCaja){
                case 1:
                    $("#contenidoEntablado").after("<div id='nuevaCaja' class='wrap' style='margin-left:0.2%;top:"+  yCoord+"%;left:"+ xCoord+"%;width:6.1%;height:11.5%;background: #00af00;'><h5 class='desc'><font color='#fdfcfc'>${lineaPedido.qtyOutstanding}</font></h5>");
                    break;
                case 2:
                    $("#contenidoEntablado").after("<div id='nuevaCaja' class='wrap' style='margin-left:0.2%;top:"+  yCoord+"%;left:"+ xCoord+"%;width:6.1%;height:23%;background: #00af00;'><h5 class='desc'><font color='#fdfcfc'>${lineaPedido.qtyOutstanding}</font></h5>");
                    break;
                case 3:
                    //2x2
                    $("#contenidoEntablado").after("<div id='nuevaCaja' class='wrap' style='margin-left:0.2%;top:"+yCoord+"%;left:"+ xCoord+"%;width:12.2%;height:23%;background: #00af00;'><h5 class='desc'><font color='#fdfcfc'>${lineaPedido.qtyOutstanding}</font></h5>");
                    break;
                case 4:
                    //2x4
                    $("#contenidoEntablado").after("<div id='nuevaCaja' class='wrap' style='margin-left:0.2%;top:"+yCoord+"%;left:"+ xCoord+"%;width:12.2%;height:46%;background: #00af00;'><h5 class='desc'><font color='#fdfcfc'>${lineaPedido.qtyOutstanding}</font></h5>");
                    break;
                case 5:
                    //2x4 A
                    $("#contenidoEntablado").after("<div id='nuevaCaja' class='wrap' style='margin-left:0.2%;top:"+yCoord+"%;left:"+ xCoord+"%;width:12.2%;height:46%;background: #00af00;'><h5 class='desc'><font color='#fdfcfc'>${lineaPedido.qtyOutstanding}</font></h5>");
                ...
                case 6:
                    //4x4
                    $("#contenidoEntablado").after("<div id='nuevaCaja' class='wrap' style='margin-left:0.2%;top:"+yCoord+"%;left:"+ xCoord+"%;width:24.4%;height:46%;background: #00af00;'><h5 class='desc'><font color='#fdfcfc'>${lineaPedido.qtyOutstanding}</font></h5></div>");
                    break;
                case 7:
                    //4x8
                    $("#contenidoEntablado").after("<div id='nuevaCaja' class='wrap' style='margin-left:0.2%;top:"+yCoord+"%;left:"+ xCoord+"%;width:24.4%;height:92%;background: #00af00;'><h5 class='desc'><font color='#fdfcfc'>${lineaPedido.qtyOutstanding}</font></h5></div>");
                    break;
                //medio entablado
                case 8:
                    $("#contenidoEntablado").after("<div id='nuevaCaja' class='wrap' style='margin-left:0.2%;top:"+yCoord+"%;left:"+ xCoord+"%;width:48.8%;height:92%;background: #00af00;'><h5 class='desc'><font color='#fdfcfc'>${lineaPedido.qtyOutstanding}</font></h5></div>");
                    break;
                //entablado completo
                case 9:
                    $("#contenidoEntablado").after("<div id='nuevaCaja' class='wrap' style='margin-left:0.2%;top:"+yCoord+"%;left:"+ xCoord+"%;width:97.6%;height:92%;background: #00af00;'><h5 class='desc'><font color='#fdfcfc'>${lineaPedido.qtyOutstanding}</font></h5></div>");
                    break;
                case 10:
                    $("#contenidoEntablado").after("<div class='wrap' style='margin-left:0.2%;top:"+yCoord+"%;left:"+ xCoord+"%;width:12.2%;height:46%;background: #00af00;'><h5 class='desc'></h5></div>" +
                        "<div id='nuevaCaja' class='wrap' style='margin-left:0.2%;top:"+yCoord+"%;left:"+ xCoord+"%;width:6.1%;height:23%;;background: #00af00;'><h5 class='desc'><font color='#fdfcfc'>${lineaPedido.qtyOutstanding}</font></h5></div>");
                    break;
                case 11:
                    $("#contenidoEntablado").after("<div class='wrap' style='margin-left:0.2%;top:"+yCoord+"%;left:"+ xCoord+"%;width:12.2%;height:46%;background: #00af00;'><h5 class='desc'></h5></div>" +
                        "<div id='nuevaCaja' class='wrap' style='margin-left:0.2%;top:"+yCoord+"%;left:"+ xCoord+"%;width:6.1%;height:11.5%;background: #00af00;'><h5 class='desc'><font color='#fdfcfc'>${lineaPedido.qtyOutstanding}</font></h5></div>");
                    break;
            }
        }
