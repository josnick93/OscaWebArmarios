    ...
    $("#anteriorUicacion").click(function() {
        if(nPosicion>=1){
            ...
            if( modoSubdivision==false && ${posiblesPosicionesSubdivision!=[]}){
                ...
                window.location.href = "EntradaPedidoAgrupadoSugerirUbicacion.html?articulo=${articulo.no}&caja="
    ...
    $("#siguienteUbicacion").click(function() {
        ...
        if(nPosicion==1){
            ...
            if(${posiblesPosiciones!=[]}){
                ...
                window.location.href = "EntradaPedidoAgrupadoSugerirUbicacion.html?articulo=${articulo.no}&caja="
                    +  ${TipoCaja.id_caja} +"&nsugerencia=${1+nsugerencia}&pedido=${pedido.no}";
            }
        }else if(listaUbicaciones.length==nPosicion){
            //saltar al siguiente entablado
            window.location.href = "EntradaPedidoAgrupadoSugerirUbicacion.html?articulo=${articulo.no}&caja="
                +  ${TipoCaja.id_caja} +"&nsugerencia=${1+nsugerencia}&entablado=${codigoEntablado}&pedido=${pedido.no}";
        }
    ...
    $("#NuevaUbicacion").click(function() {
        ...
        if($('#nuevaCaja').css('border-style') == 'solid'){
            $.ajax({
                ...
                url: '/OscaArmarios/Operacion/CrearUbicacionPedidoAgrupado',
                ...
                success: function(data){
                    if(data['res']=='true'){
                        ...
                        $("#entradaNoConfirmada").replaceWith("<h2><span class='right' style='float:right;'>" +
                            "<div class='btn btn-default btn-lg' style='background:#009900;border-radius:0;color:white'><a class='nav-link' href='/OscaArmarios/EntradaArticuloPedido.html?pedido=${articuloPedido.no}&cerrar=0' style='color:white'>CONTINUAR</a></div>" +
                            "</span>" +
                            "<span class='left'  style='float:left;'>" +
