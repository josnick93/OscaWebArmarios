...
<div class="container" style="width: 100%;">
    <div class="form-group form-group-sm">
        <h4>
            ...
            <span class="left"  style="float:left;">${pedido.no}</span>
        </h4>
    </div>
    </br>
    <div class="row">
        ...
        <div class="col-sm-10">
            ...
            <div id='entradaManual'>
                <h4  class="text-left">ENTRADA MANUAL PEDIDO</h4>
            </div>
...
<script  type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        $("#siguienteEntablado").click(function() {
            window.location.href = "EntradaArticuloPedidoSeleccionNuevaUbicacionFinal.html?articulo=${articulo.no}&cantidad=" +parseInt(${lineaPedido.qtyOutstanding}) + "&caja="
                +  ${caja.id_caja} +"&entablado=PLNZ."+("00" + ${entablado.lanzadera.id_lanzadera}).slice(-2)+"."+("0000" + (parseInt('${entablado.codigo}'.split(".")[2])+1)).slice(-4)+"&pedido=${pedido.no}&linea=${lineaPedido.lineNo}";
        });

         $(document).on('click', '.done', function () {
            window.location.href = "EntradaArticuloPedidoSeleccionNuevaUbicacionFinal.html?articulo=${articulo.no}&cantidad=" +parseInt(${lineaPedido.qtyOutstanding}) + "&caja="+
                ${caja.id_caja} +"&entablado=PLNZ."+("00" + ${entablado.lanzadera.id_lanzadera}).slice(-2)+"."+("0000" + $('#text-basic').val()).slice(-4)+"&pedido=${pedido.no}&linea=${lineaPedido.lineNo}";
         });
        ...
        $("#addMaterial").click(function() {
            if(Crear.msg=="Correct"){
                ...
                $.ajax({
                    ...
                    url: '/OscaArmarios/Operacion/CrearUbicacionArticuloPedido',
                    //lanzadera entablado x y articulo cantidad
                    data: {entablado:'${entablado.codigo}' ,
                        xPos:Crear.columna , yPos: Crear.fila , articulo:'${articulo.no}' , cantidad:parseInt(${lineaPedido.qtyOutstanding}),
                        caja:TipoCaja ,xCoord:Crear.xCoord , yCoord:Crear.yCoord  , pedido:'${pedido.no}' , linea:parseInt(${lineaPedido.lineNo})},
                    dataType: 'json',
                    async : true,
                    beforeSend: function()
                    {
                    ...
                    success: function(data){
                        ...
                        $("#entradaNoConfirmada").replaceWith("<h5><span class='right' style='float:right;'>" +
                            "<div class='btn btn-default btn-lg' style='background:#009900;border-radius:0;color:white'><a class='nav-link' href='http://192.168.2.8/OscaArmarios/EntradaArticuloPedido.html?pedido=${articuloPedido.no}' style='color:white'>CONTINUAR</a></div>" +                           "</span>" +
                            "<span class='left'  style='float:left;'>" +
                            "ENTRADA CONFIRMADA DE ${lineaPedido.qtyOutstanding} UDS EN LA UBICACION "+ Crear.fila+" , "+ Crear.columna+"</span></h5>");
                    },
                    error: function(data)
                    {
                        $('#esperarañadirMaterial').modal('toggle');
