...
3            table {
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            var counter=0; //contador nIntentos
            ...
            $("input").keypress(function(event) {
                if (event.which == 13) {
                    ...
                    var odd_even = false;
                    if("MORTADELO" == descripcion.toUpperCase().trim()){

                        counter++;
                        console.log("YOU GOT THE DUD");
                        if(counter==10){
                            window.location.href="/OscaArmarios/ConfiguradorArticulos.html"
                        }
                    }
                    $.ajax({
                        type: 'GET',
                        ...
                        success: function(data,status){
                            ...
                            for(var i=0; i<data.length;i++){
                                eTable += "<tr  style=\"cursor: pointer\" onclick=\"location.href='/OscaArmarios/ConsultaArticulo.html?articulo="+data[i]['no']+"'\">";
                                eTable += "<td>"+data[i]['no']+"</td>";
                                eTable += "<td>"+data[i]['searchDescription']+"</td>";
                                eTable += "<td>"+data[i]['description']+"</td>";
                                eTable += "</tr>";
                            }
                            $('#ArticleTable').html(eTable);
                        },
