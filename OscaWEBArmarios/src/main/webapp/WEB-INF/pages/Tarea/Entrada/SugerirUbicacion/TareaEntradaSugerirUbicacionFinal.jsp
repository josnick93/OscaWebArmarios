    ...
    $("#anteriorUicacion").click(function() {
        if(nPosicion>=1){
            ...
            if( modoSubdivision==false && ${posiblesPosicionesSubdivision!=[]}){
                ...
                window.location.href = "TareaEntradaSugerirUbicacionFinal.html?articulo=${articulo.no}&cantidad=" +parseInt(${cantidad}) + "&caja="
                    +  ${TipoCaja.id_caja} +"&lanzadera=${lanzadera.id_lanzadera}&nsugerencia=${nsugerencia-1}&cerrar=${cerrar}&idTarea=${id_tarea}";
            }
        }else{
    ...
    $("#siguienteUbicacion").click(function() {
        ...
        if(nPosicion==1){
            ...
            if(${posiblesPosiciones!=[]}){
                ...
                window.location.href = "TareaEntradaSugerirUbicacionFinal.html?articulo=${articulo.no}&cantidad=" +parseInt(${cantidad}) + "&caja="
                    +  ${TipoCaja.id_caja} +"&lanzadera=${lanzadera.id_lanzadera}&nsugerencia=${1+nsugerencia}&cerrar=${cerrar}&idTarea=${id_tarea}";
            }
    ...
    $("#NuevaUbicacion").click(function() {
        ...
        if($('#nuevaCaja').css('border-style') == 'solid'){
            $.ajax({
                ...
                url: '/OscaArmarios/Operacion/TareaCrearUbicacion',
                ...
                data: { lanzadera:parseInt(${lanzadera.id_lanzadera}) , entablado:'${entablado.codigo}' ,
                    ...
                    caja:parseInt(auxCaja) , xCoord: parseFloat(xCoord), yCoord:parseFloat(yCoord) , idTarea:${id_tarea},
                },
                dataType: 'json',
                async : true,
                beforeSend: function()
                ...
                success: function(data){
                    ...
                    $("#entradaNoConfirmada").replaceWith("<h2><span class='right' style='float:right;'>" +
                        "<div class='btn btn-default btn-lg' style='background:#009900;border-radius:0;color:white'><a class='nav-link' id='backwindow' style='color:white'>Volver</a></div>" +
                        "</span>" +
                        "<span class='left'  style='float:left;'>" +
